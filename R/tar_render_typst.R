...
tar_render_typst <- function(
  ...
  name <- stringr::str_glue("rpt_{name}")
  ...
  values <- targets::tar_tidy_eval(
    as.expression(substitute(write_to_json(values, "values"))),
    envir,
    TRUE
  )
  ...
  deps <- targets::tar_tidy_eval(
    substitute(list(
      values = values,
      appendix = appendix,
      appendix_template = appendix_template
    )),
    envir,
    TRUE
  )

  figs <- targets::tar_tidy_eval(substitute(figs), envir, TRUE)
  tbls <- targets::tar_tidy_eval(substitute(tbls), envir, TRUE)

  garbage_collection <- isTRUE(garbage_collection)

  command <- targets::tar_tidy_eval(
    as.expression(substitute(render_manuscript(
      path = filename,
      deps = deps,
      fig = figs,
      tbl = tbls,
