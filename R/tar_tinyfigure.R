...
#' @param command R code to run the target. This should return a `ggplot2` plot
#'     object.
#' @param filename String, filename stub for the saved image files.
#' @param ... Additional arguments passed on to [save_plot()].
...
tar_tinyfigure <- function(
  ...
  command <- targets::tar_tidy_eval(
    as.expression(substitute(save_tinyplot(command, filename, ...))),
    envir,
    tidy_eval
  )
